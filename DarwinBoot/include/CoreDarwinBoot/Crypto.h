...
typedef struct _CDBSHA1Context *CDBSHA1ContextRef;

// effectively a one-shot operation
errno_t CDBSHA1Digest(const void *Data, size_t DataLength, void *Digest);

// create + init the SHA-1 context
CDBSHA1ContextRef CDBSHA1CreateContext(void);

errno_t CDBSHA1UpdateDigest(CDBSHA1ContextRef Context, const void *Data, size_t DataLength);

errno_t CDBSHA1FinalizeDigest(CDBSHA1ContextRef Context, void *Digest);

void CDBSHA1DestroyContext(CDBSHA1ContextRef Context);

typedef struct _CDBSHA256Context *CDBSHA256ContextRef;
